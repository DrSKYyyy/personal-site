...
如果你有一组有逻辑顺序的文章（如 Unity 教程），可以在 `src/content/blog/` 下创建**子文件夹**来组织：

```
src/content/blog/
├── unity超详细教程/        ← 文件夹名即为系列名
│   ├── 入门篇.md
│   ├── 脚本基础.md
│   └── 附录-常见问题.md
├── 我的第一篇文章.md        ← 根目录下的是独立文章
└── 第二篇文章.md
```

- 子文件夹内的文章 → 自动归为一个**系列**，在文章列表中显示为一个卡片，点击进入系列页面
- 根目录下的 `.md` 文件 → 保持现有的独立文章列表形式
...
title: "文章标题"
date: "2025-01-01"
series: "unity超详细教程"      # 通常由脚本自动从文件夹名识别，无需手动写
seriesTitle: "Unity超详细教程" # 系列显示名称（可选，不填则用文件夹名）
seriesOrder: 1               # 在系列中的顺序（数字越小越靠前）
seriesSection: "主线教程"     # 分组标签（可选，用于在系列内分组，如"主线教程"、"附录"）
---
```

| 字段 | 必填 | 说明 |
|------|------|------|
| `series` | 脚本自动生成 | 系列标识符（文件夹名），用于将文章归为一个系列 |
| `seriesTitle` | 可选 | 系列在页面上显示的名称，不填则显示文件夹名 |
| `seriesOrder` | 可选 | 在系列内的排序序号（1、2、3...），不填则按文件顺序 |
| `seriesSection` | 可选 | 在系列内部分组，如 `主线教程`、`附录`。侧边栏会按此分组显示标题 |

**系列页面效果：** 访问 `https://xxs.beauty/tutorial/系列名` 可看到带侧边栏目录和上下篇导航的教程页面。

> 💡 **自动补全：** 构建时脚本会自动补全缺失的 `title`（从正文标题提取）、`date`（文件修改时间）、`series`（从文件夹名识别）。从 Obsidian 直接复制文件夹过来即可，无需手动调整前沿。
